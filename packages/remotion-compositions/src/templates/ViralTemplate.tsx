...
export function ViralTemplate({
  ...
  const scale = interpolate(frameInChunk, [0, entryDuration], [0.9, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
    ...
    >
      ...
      >
        {currentChunk.map((word, wordIndex) => (
          <div key={`${word}-${wordIndex}-${currentChunkIndex}`}>
            <div
              style={{
                backgroundColor: brandKit?.primaryColor ?? '#FFD700',
                padding: '8px 20px',
                borderRadius: '8px',
            ...
            >
              <p
                style={{
                  color: brandKit?.textColor ?? '#000000',
                  fontSize: '42px',
                  fontWeight: 900,
                  textAlign: 'center',
                  margin: 0,
